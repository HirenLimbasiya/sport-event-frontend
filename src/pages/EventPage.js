...
const EventPage = () => {
    ...
    const handleReject = async (id, name) => {
        const response = await fetch(`http://localhost:5000/remove/${id}/${name}`, {
            method: "PUT",
            headers: {
                authorization: `${JSON.parse(localStorage.getItem('token'))}`
            }
                ...
                {singleData && <>
                    <div className='single-page-container'>
                        ...
                        <div style={{ marginLeft: "20px" }}>
                            ...
                            <p>Available Seats: {singleData.totalSeats - singleData.currentParticipants.length}</p>
                        </div>
                    </div>
                    ...
                    <div className='participant-list'>
                        {
                            singleData.currentParticipants.length > 0 ? singleData.currentParticipants.map((name, index) => <div>{name}</div>) : <h3>No Participant</h3>
                        }
                    </div>
                                ...
                                singleData.pending.map((name, index) => {
                                    return (
                                        <div>
                                            <p>{name}</p>
                                            {
                                                singleData.totalSeats - singleData.currentParticipants.length === 0 ? <button className='btn-accept-disable' disabled>Accept</button> : <button className='accept-btn' onClick={() => handleAccept(singleData._id, name)}>Accept</button>
                                            }

                                            <button className='reject-btn' onClick={() => handleReject(singleData._id, name)}>Reject</button>
                                        </div>
                                    )
                                })
