...
const Events = ({ eventFunction }) => {
  ...
  const fetchEvents = async (name) => {
    const response = await fetch(
      `${process.env.REACT_APP_BASE_URL}/events/${name}`,
      {
        method: "GET",
        headers: {
    ...
    const ans = data.filter((event) => {
      if (
        event.pending.includes(name) === false &&
        event.currentParticipants.includes(name) === false
      ) {
        return event;
      }
    });
  ...
  const handleRequest = async (id) => {
    ...
    const response = await fetch(
      `${process.env.REACT_APP_BASE_URL}/pending/${id}`,
      {
        method: "PUT",
  ...
  async function handleSearch(e) {
    ...
    if (key) {
      const response = await fetch(
        `${process.env.REACT_APP_BASE_URL}/events/${local.name}/${key}`,
        {
          method: "GET",
          headers: {
            authorization: `${JSON.parse(localStorage.getItem("token"))}`,
            ...
            events.map((event) => {
              return (
                <div key={event._id} className="single-event-card">
                  ...
                  {event.totalSeats - event.currentParticipants.length === 0 ? (
